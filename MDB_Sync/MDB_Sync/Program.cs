...
namespace OledbConnection
{
    ...
    class Program
    {
        ...
        static async Task Main()
        {
            ...
            if (!HasMdbExtension(serverDbPath))
            {
                while (true)
                {
                    // If server path is a folder, ask user to select a file
                    string fileName = Path.GetFileName(serverDbPath);
                    if (string.IsNullOrEmpty(fileName))
                    {
                        Console.WriteLine($"Server path is a folder: {serverDbPath}");
                        Console.Write("Enter the MDB filename to sync (e.g., database.mdb): ");
                        fileName = Console.ReadLine();
                        serverDbPath = Path.Combine(serverDbPath, fileName);
                    }

                    Console.Write("Enter destination folder (e.g., C:\\SyncFolder): ");
                    string destFolder = Console.ReadLine();

                    if (!Directory.Exists(destFolder))
                    {
                        Directory.CreateDirectory(destFolder); // Create if doesn't exist
                        Console.WriteLine($"Created destination folder: {destFolder}");
                    }

                    // Set client path to destination folder + filename
                    clientDbPath = Path.Combine(destFolder, Path.GetFileName(serverDbPath));

                    RunCommand($"net use {DRIVE_LETTER} /delete", false);

                    Console.WriteLine($"Mounting shared folder {Path.GetDirectoryName(serverDbPath)}...");
                    var connectCmd = $"net use {DRIVE_LETTER} {Path.GetDirectoryName(serverDbPath)} /user:{USERNAME} {PASSWORD} /persistent:no";
                    if (!RunCommand(connectCmd))
                    {
                        Console.WriteLine("ERROR: Failed to connect to shared folder.");
                        continue;
                    ...
                    Console.WriteLine($"Copying {Path.GetFileName(serverDbPath)} from server...");
                    try
                    {
                        File.Copy(serverFilePath, clientDbPath, true);
                        Console.WriteLine($"Successfully copied to: {clientDbPath}");
                        break;
                    }
                    catch (Exception ex)
                    {
