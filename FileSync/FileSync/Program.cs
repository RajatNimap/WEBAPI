    ...
    class Program
    {
        ...
        static void SyncFiles(string sourceFolder, string targetFolder, string logFile, string direction, List<string>? excludeFolders, bool isFullServerToClient)
        {
            ...
            foreach (var src in sourceFiles)
            {
                ...
                DateTime srcWriteTime = File.GetLastWriteTimeUtc(src);
                if (srcWriteTime <= _lastSyncTime && File.Exists(dest))
                    continue;

                bool shouldCopy = !File.Exists(dest) || srcWriteTime > File.GetLastWriteTimeUtc(dest);
                if (shouldCopy)
                {
                    File.Copy(src, dest, true);
                    PrintSuccess($"  [✓] Copied: {relativePath} {direction}");
        ...
        static void SyncFilesBothDirections()
        {
            try
            {
                if (_syncRunning)
                {
                    ...
                    if (Directory.GetLastWriteTimeUtc(serverFolder) > _lastSyncTime)
                    {
                        SyncFiles(serverFolder, clientPath, logFilePath, "ServerToClient", excludeList, true);
                    }
