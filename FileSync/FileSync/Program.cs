  ...
  public class Program
    {
        ...
        static void SyncFiles(string sourceFolder, string targetFolder, string logFile, string direction, bool isFullServerToClient)
        {
            ...
            // Get only files that changed since last sync
            var sourceFiles = Directory
                .EnumerateFiles(sourceFolder, "*", SearchOption.AllDirectories)
                .Where(file =>
                {
                    ...
                    if (!metadata.Files.TryGetValue(relativePath, out var fileMeta))
                        return true;

                    return lastWriteTime > fileMeta.LastModified ||
                           new FileInfo(file).Length != fileMeta.FileSize;
                })
                .ToArray();
           ...
           var sourceDirs = Directory.EnumerateDirectories(sourceFolder, "*", SearchOption.AllDirectories);
            ...
            foreach (var src in sourceFiles)
            {
                ...
                bool shouldCopy = !File.Exists(dest) ||
                                 ...
                                 new FileInfo(src).Length != (File.Exists(dest) ? new FileInfo(dest).Length : -1);
                ...
                if (shouldCopy)
                {
                    ...
                    // Update metadata
                    metadata.Files[relativePath] = new FileMetadata
                    {
            ...
            metadata.LastSyncTime = currentSyncTime;
        ...
        static SyncMetadata LoadSyncMetadata()
        {
            string metaFilePath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, syncMetaFile);

            if (!File.Exists(metaFilePath))
                return new SyncMetadata { LastSyncTime = DateTime.MinValue };

            try
            {
                string json = File.ReadAllText(metaFilePath);
                return JsonSerializer.Deserialize<SyncMetadata>(json) ?? new SyncMetadata { LastSyncTime = DateTime.MinValue };
            }
            catch
            {
                return new SyncMetadata { LastSyncTime = DateTime.MinValue };
        ...
        static void SaveSyncMetadata(SyncMetadata metadata)
        {
            try
            {
                string metaFilePath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, syncMetaFile);
            ...
            {
                PrintError($"Failed to save sync metadata: {ex.Message}");
            }
        }
        ...
        //                SyncFiles(clientFolder, correspondingServerFolder, logFilePath, "ClientToServer", false);
        //                SyncFiles(correspondingServerFolder, clientFolder, logFilePath, "ServerToClient", false);
        //            }
        ...
        static void CleanupDeletedFiles(string sourceFolder, string targetFolder)
        {
            var metadata = LoadSyncMetadata();
            var filesToRemove = new List<string>();

            // Check target directory for files that no longer exist in source
            if (Directory.Exists(targetFolder))
            {
                var targetFiles = Directory.EnumerateFiles(targetFolder, "*", SearchOption.AllDirectories);

                foreach (var targetFile in targetFiles)
                {
                    string relativePath = Path.GetRelativePath(targetFolder, targetFile);
                    string sourceFile = Path.Combine(sourceFolder, relativePath);

                    if (!File.Exists(sourceFile) && metadata.Files.ContainsKey(relativePath))
                    {
                        File.Delete(targetFile);
                        filesToRemove.Add(relativePath);
                        PrintWarning($"  [-] Deleted: {relativePath} (no longer exists in source)");
                    }
                }
            }
